...
- In the GUI, `MainWindow._rag_index` caches the `VectorIndex`; it is set via the worker's `index_ready` signal and invalidated on file change, embed-model change, and new session. The worker must receive `rag_index=` on turns 2+ or retrieval won't fire. Loading a saved session leaves the index `None` until the next file change / new session (known limitation — don't rebuild on load, it would wipe replayed history).
